...
export function SafetyOverlay({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      />
      <div className="relative w-full max-w-md animate-rise rounded-3xl bg-cream p-6 shadow-2xl ring-1 ring-line sm:p-7">
        <a
          href={QUICK_EXIT_HREF}
          className="absolute right-4 top-4 rounded-full bg-sand px-2.5 py-1 text-[0.7rem] font-medium text-ink-soft transition-colors hover:bg-line"
        >
          Quick exit
        </a>
        ...
        <div
          className={`flex h-11 w-11 items-center justify-center rounded-2xl ${
            urgent ? "bg-alert-soft text-alert" : "bg-mint text-forest"
          }`}
        >
        ...
        <p className="mt-2 text-[0.98rem] leading-relaxed text-ink-soft">{copy.body}</p>
        ...
        <div className="mt-5 space-y-2.5">
          {CRISIS_RESOURCES.map((r) => (
            <a
              key={r.name}
              href={r.href}
              className="flex items-center gap-3 rounded-2xl border border-line bg-white/80 p-3.5 transition-colors hover:border-feather"
            >
        ...
        <p className="mt-4 text-[0.7rem] leading-relaxed text-ink-faint">{SAFETY_DISCLAIMER}</p>
      </div>
    </div>
  );
}
